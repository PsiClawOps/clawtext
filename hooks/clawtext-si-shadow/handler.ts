...
function resolveSIDistPath(): string {
  const candidates = [
...
const _workspaceEngines = new Map<string, SIEngine>();
let _siModuleLoaded: boolean | null = null; // null = not yet attempted
...
const _loggedEventShapes = new Set<string>();
 ...
 */
function resolveWorkspaceForAgentId(agentId: string): string {
  try {
    if (!fs.existsSync(OPENCLAW_CONFIG_PATH)) return DEFAULT_WORKSPACE;
    ...
    const agentsList = cfg?.agents?.list as Array<Record<string, unknown>> | undefined;
    ...
    const agentsEntries = cfg?.agents?.entries as Record<string, Record<string, unknown>> | undefined;
 ...
 */
function resolveWorkspace(sessionKey: string): string {
  if (workspaceCache.has(sessionKey)) return workspaceCache.get(sessionKey)!;
 ...
 */
async function getWorkspaceEngine(workspacePath: string): Promise<SIEngine | null> {
  const cached = _workspaceEngines.get(workspacePath);
  if (cached) return cached;

  // Only attempt to load the module once
  if (_siModuleLoaded === false) return null;
  ...
  try {
    ...
    let siConfig: Record<string, unknown> = {};
    try {
      const cfg = JSON.parse(fs.readFileSync(OPENCLAW_CONFIG_PATH, 'utf-8'));
 ...
 */
function extractMessage(event: Record<string, unknown>): AgentMessage | null {
...
const handler = async (
  ...
  try {
    ...
    const hookCtx = (ctx ?? (event.context as Record<string, unknown> | undefined) ?? {}) as {
      sessionKey?: string;
      sessionId?: string;
      agentId?: string;
      workspaceDir?: string;
    ...
    if (event.isHeartbeat || hookCtx.isHeartbeat) return;

    const sessionKey = hookCtx.sessionKey || hookCtx.sessionId || (event.sessionKey as string | undefined) || '';
    if (!sessionKey) {
    ...
    if (!_loggedEventShapes.has(sessionKey)) {
      ...
      for (const k of Object.keys(event)) {
        ...
        shape[k] = Array.isArray(v) ? `Array(${(v as unknown[]).length})` : typeof v;
      }
    ...
    const workspacePath = (hookCtx.workspaceDir as string | undefined) || resolveWorkspace(sessionKey);
