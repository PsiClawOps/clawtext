...
# New governance flow
# 1) Plan + stage (default)
_UPGRADE_FORCE_ESCAPE=1 bash skills/upgrade/scripts/safe-upgrade.sh

# 2) Execute staged upgrade
_UPGRADE_FORCE_ESCAPE=1 bash skills/upgrade/scripts/safe-upgrade.sh --live

# 3) Emergency override (no stage required, skips latest-version short-circuit)
_UPGRADE_FORCE_ESCAPE=1 bash skills/upgrade/scripts/safe-upgrade.sh --force --live

# Legacy: pre-flight only (no changes)
bash skills/upgrade/scripts/safe-upgrade.sh --check      # Pre-flight only

# Legacy: rollback (requires --live unless --force)
bash skills/upgrade/scripts/safe-upgrade.sh --rollback --live  # Rollback from last backup
```
...
2. Inform user: "Preparing upgrade — planning and staging required."
3. Stage maintenance flow:
   ```bash
   _UPGRADE_FORCE_ESCAPE=1 bash skills/upgrade/scripts/safe-upgrade.sh
   ```
4. Execute staged upgrade:
   ```bash
   _UPGRADE_FORCE_ESCAPE=1 bash skills/upgrade/scripts/safe-upgrade.sh --live
   ```
