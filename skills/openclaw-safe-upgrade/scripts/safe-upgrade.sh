...
#
# Usage: bash safe-upgrade.sh [--check|--plan] [--rollback] [--live] [--force]
#   --plan, --check   = pre-flight check only (no staging)
#   --live            = execute staged operation
#   --force           = emergency override (also skips latest-version short-circuit)
#   --rollback        = manual rollback (requires --live unless --force)
#   default (no flags)= run preflight gate and stage for later execution
...
# ─────────────────────────────────────────────
# ClawLogix governance gate
# ─────────────────────────────────────────────
GOV_SCRIPT="${OPENCLAW_RESTART_GOVERNANCE_SCRIPT:-$HOME/.openclaw/workspace/scripts/restart-governance.sh}"
PLAN_MODE=0
ROLLBACK_MODE=0
RUN_LIVE=0
FORCE_UPGRADE=0
GOV_REASON_PREFIX="${OPENCLAW_RESTART_GOV_REASON_PREFIX:-openclaw-safe-upgrade}"

governance_gate() {
  local action="$1"

  if [ "$FORCE_UPGRADE" -eq 1 ]; then
    RESTART_GOV_ACTOR="${OPENCLAW_RESTART_GOV_ACTOR:-operator}" \
      RESTART_GOV_REASON="$action" \
      RESTART_GOV_SOURCE="safe-upgrade" \
      RESTART_GOV_COMMAND="true" \
      "$GOV_SCRIPT" --force --live --assume-yes
    return $?
  fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --force)
      ...
      ;;
    *)
      echo "Usage: safe-upgrade.sh [--plan|--check] [--rollback] [--live] [--force]"
      echo ""
      echo "  --plan, --check   Pre-flight check only (no stage or execution)"
      echo "  --rollback        Rollback from last upgrade (requires --live unless --force)"
      echo "  --live            Execute staged operation"
      echo "  --force           Emergency override and also upgrade even if already latest"
      echo "  default (no flags) Stage required maintenance action via governance gate"
      exit 1
      ;;
  esac
  shift
...
if [ "$ROLLBACK_MODE" -eq 1 ]; then
  governance_gate "${GOV_REASON_PREFIX}:rollback"
  do_rollback "manual"
  exit 0
fi

if [ "$FORCE_UPGRADE" -eq 1 ]; then
  governance_gate "${GOV_REASON_PREFIX}:upgrade-force"
  do_upgrade "force"
  exit 0
fi

# Default path: plan, stage, then wait for explicit --live execution
if [ "$RUN_LIVE" -eq 1 ]; then
  governance_gate "${GOV_REASON_PREFIX}:upgrade"
  do_upgrade
  exit 0
fi
